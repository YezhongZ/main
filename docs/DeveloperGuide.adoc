...
1. Go to ConnectToGoogleeCalendarTest.Java
...
Change the line assertEquals("[Your Google Email Address]", getFromGoogleCalendar.getId());

==== Setup for welcome page

Go to BrowserPanel.java

Locate the line with a delay time
This number shall be a large number (such as `999999000`)
When deployed to user, please change to an expected time for user to see the welcome page (such as `45000` for 45 seconds)
...
**Step 5** +
-> For each event extracted, various ConvertEvent utility methods will be called `addGcEventToLocal` is called to convert the Google Extracted Event to local Event.
